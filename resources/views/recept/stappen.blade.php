    ...
    @foreach($recipe->steps as $step)
        <div class="bg-white border-2 border-black shadow-[3px_3px_0px_0px_#000] flex overflow-hidden">
            <div class="relative flex-shrink-0 w-44 h-28 border-r-2 border-black">
                ...
                <span class="absolute bottom-1 right-1 text-[7px] font-black bg-black text-white px-1 py-0.5">
                    {{ $step->video_timestamp ?? '0:18' }}
                </span>
            </div>
            <div class="flex-1 p-4">
                <div class="flex items-center justify-between mb-3">
                    <span class="w-9 h-9 rounded-full border-2 border-black flex items-center justify-center text-sm font-black flex-shrink-0">{{ $step->step_number }}</span>
