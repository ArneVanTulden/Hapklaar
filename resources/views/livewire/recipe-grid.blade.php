<div x-data="{ filtersOpen: false }"
     class="max-w-6xl mx-auto px-4 md:px-6 flex flex-col md:flex-row gap-5 md:gap-7 md:items-start">
    ...
    @php
        ...
        $filterCount = count($diets) + ($maxCal < 1500 ? 1 : 0) + ($maxAfwas !== null ? 1 : 0);
    @endphp
    ...
    <button type="button"
        ...
        @if($filterCount > 0)
            <span class="bg-[var(--lime)] border-2 border-black text-[10px] font-black px-2 py-0.5">{{ $filterCount }}</span>
        @endif
    </button>
    ...
    >
        ...
        <div class="mb-6">
            ...
            <div class="space-y-3">
                @foreach($dietOptions as $diet)
                    <label wire:click.prevent="toggleDiet('{{ $diet['value'] }}')" class="flex items-center gap-3 cursor-pointer select-none">
                        <span class="w-5 h-5 border-2 border-black flex items-center justify-center flex-shrink-0 bg-white">
                            @if(in_array($diet['value'], $diets))
                                <svg class="w-3 h-3" viewBox="0 0 12 12" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round">
                                    <path d="M2 6l3 3 5-5"/>
                                </svg>
                            @endif
                        </span>
        ...
        <div class="mb-6">
            ...
            <input type="range" min="500" max="1500" step="25"
                   wire:model.live.debounce.400ms="maxCal"
                   class="w-full h-1.5 accent-black cursor-pointer mb-2.5">
            <span class="{{ $maxCal > 1000 ? 'bg-red-400' : ($maxCal > 700 ? 'bg-orange-400' : 'bg-[var(--lime)]') }} inline-block border-2 border-black px-2 py-0.5 text-[9px] font-black uppercase tracking-widest">
                MAX {{ $maxCal }} KCAL
            </span>
        </div>
        ...
        <div class="mb-7">
            ...
            <div class="flex gap-1.5">
                @for($i = 1; $i <= 5; $i++)
                    <button type="button"
                            wire:click="setAfwas({{ $i }})"
                            class="{{ $maxAfwas !== null && $i <= $maxAfwas ? 'bg-[var(--lime)]' : 'bg-white hover:bg-[var(--pink-soft)]' }} w-9 h-9 border-2 border-black font-black text-sm transition-colors">
                        {{ $i }}
                    </button>
                @endfor
            </div>
        ...
        <div class="flex gap-5 md:gap-7 border-b-2 border-black mb-6 overflow-x-auto md:overflow-visible"
            ...
            @foreach(['popular' => 'POPULAIR', 'snel' => 'SNEL', 'gezond' => 'GEZOND'] as $key => $label)
                <button wire:click="setSort('{{ $key }}')"
                        @class([
                            'text-[11px] font-black uppercase tracking-widest pb-2.5 -mb-px transition-colors',
                            'border-b-2 border-brand text-brand' => $sort === $key,
                            'text-gray-400 hover:text-black' => $sort !== $key,
                        ])>{{ $label }}</button>
            @endforeach
        </div>
